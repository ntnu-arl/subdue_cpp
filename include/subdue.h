...
#define SUBSTITUTE_VERTEX_LABEL_COST   4.0 // substitute vertex label
...
#define POSE_COST_MULTIPLIER           2.0
...
// double * label_wise_mean_degree;
// int * label_wise_count;
...
typedef struct 
{
   ...
   ULONG level;            // determines if the vertex/edge with this label is a substructure or primary element
                           // level = 0: primary, level = n: all vertices in the substructure have level < n
   BOOLEAN imp;            // If true it will be included in the initial structures
} Label;
...
typedef struct 
{
   ...
   ULONG instanceID;
   ULONG degree;   // Number of edges in the original graph (the one in which
                   // we are trying to find patterns)
} Vertex;
...
// Vertex to Edge map: Map the outgoing edges of an instance of a substructure to the internal vertex indices they connect to
...
typedef struct _instance
{
   ...
   ULONG parent_instance_id;
   gsl_vector * mean_position;
   gsl_vector * mean_orientation;
} Instance;
...
typedef struct 
{
   ...
   ULONG level;            // determines if the vertex/edge with this label is a substructure or primary element
                           // level = 0: primary, level = n: all vertices in the substructure have level < n
   char label_str[TOKEN_LEN];
   double * instance_match_costs;
} Substructure;
...
typedef struct 
{
   ...
   BOOLEAN use_pose_cost;
   BOOLEAN normalize_value_with_cost;
   double * label_wise_mean_degree;
   int * label_wise_count;
} Parameters;
...
Parameters *PostProcessParameters(Parameters *);
...
Graph * CompressFinalGraphs(Substructure *, Parameters *, ULONG, BOOLEAN);
...
void calculateMeanPositions(Graph *, InstanceList *, LabelList *);
void calculateMeanPosition(Graph *, Instance *, LabelList *);
...
void InterconnectInstances(InstanceList *, Graph *);
...
BOOLEAN GraphMatch(Graph *, Graph *, LabelList *, double, double *,
                   VertexMap *, BOOLEAN);
double InexactGraphMatch(Graph *, Graph *, LabelList *, double, VertexMap *, BOOLEAN);
...
double PoseCost(Graph *, Graph *, ULONG, ULONG, ULONG *, MatchHeapNode *);
gsl_matrix *getRotationMatrixSVD(gsl_matrix *, gsl_matrix *);
...
void crossProduct(gsl_vector *, gsl_vector *, gsl_vector *);
void printVector(gsl_vector *, int);
void printMatrix(gsl_matrix *, int, int);
void normalize(gsl_vector *);
void copyMatrix(gsl_matrix *, gsl_matrix *, int, int);
void transformToPlaneOf(gsl_vector *, gsl_vector *, gsl_vector *, gsl_vector *, gsl_vector *);
gsl_matrix *getRotationMatrixFromPlane(gsl_vector *, gsl_vector *, gsl_vector *);
gsl_matrix * recenterPoints(gsl_matrix *);
double sign(double);
...
void calculateSubLevel(Substructure *, LabelList *);
void calculateSubListLevel(SubList *, LabelList *);
...
void SubListInsert(Substructure *, SubList *, ULONG, BOOLEAN, LabelList *, BOOLEAN);
BOOLEAN MemberOfSubList(Substructure *, SubList *, LabelList *, BOOLEAN);
...
BOOLEAN InstanceListOverlap(Instance *, InstanceList *, InstanceListNode **, int *);
BOOLEAN RemoveOverlappingInstances(Instance *, InstanceList *, InstanceList *);
...
void markPatternInstances(Substructure * , Graph *, BOOLEAN val);
void markPatternInstance(Instance *, Graph *, BOOLEAN val);
