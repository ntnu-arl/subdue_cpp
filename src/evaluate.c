...
void EvaluateSub(Substructure *sub, Parameters *parameters)
{
   ...
   switch(evalMethod) 
   {
      case EVAL_MDL:
         ...
         // {
         //    mean_cost += sub->instance_match_costs[i];
         // }
         // mean_cost /= sub->numInstances;
         // if(parameters->normalize_value_with_cost)
         //    subValue /= (1.0 + mean_cost);
         
         // qsort(sub->instance_match_costs, sub->numInstances, sizeof(double), compare);
         // double median_cost = 0.0;
         // if(sub->numInstances % 2 != 0)
         // {
         //    median_cost = sub->instance_match_costs[sub->numInstances/2];
         // }
         // else
         // {
         //    median_cost = (sub->instance_match_costs[sub->numInstances/2-1] + sub->instance_match_costs[sub->numInstances/2]) / 2.0;
         // }
         // if(parameters->normalize_value_with_cost)
         //    subValue /= (1.0 + median_cost);
         // subValue = sub->numInstances;
         // subValue = sub->numInstances * (sizeOfPosGraph + sizeOfNegGraph);
