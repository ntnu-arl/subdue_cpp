                     ...
                     Parameters *parameters)
{
   ...
   // Calculate mean positions of the instance
   // printf("Calculating mean positions\n");
   calculateMeanPositions(graph, instanceList, parameters->labelList);
   // printf("Mean positions calculated\n");
   ...
   for (v = 0; v < numInstances; v++) 
   {
      ...
      compressedGraph->vertices[vertexIndex].instanceID = v;
      compressedGraph->vertices[vertexIndex].used = FALSE;
      // printf("0\n");
      compressedGraph->vertices[vertexIndex].position = gsl_vector_alloc(3);
      compressedGraph->vertices[vertexIndex].orientation = gsl_vector_alloc(3);
      // printf("1\n");
      gsl_vector_memcpy(compressedGraph->vertices[vertexIndex].position, instanceListNode->instance->mean_position);
      gsl_vector_memcpy(compressedGraph->vertices[vertexIndex].orientation, instanceListNode->instance->mean_orientation);
      // printf("Mean position of instance: "); printVector(compressedGraph->vertices[vertexIndex].position, 3);printf("\n");
      // printf("2\n");
   ...
   // printf("4\n");
   // FreeInstanceListNode(current_instance);
...
Graph * WriteCompressedGraphToFile(Substructure *sub, Parameters *parameters,
                                ULONG iteration)
{
   FILE *fp = NULL;
   ...
   /// Storing the compressed graph
   // printf("Graph written to file\n");
   // if (sub->numInstances > 0) 
   // {
   //    FreeGraph(parameters->posGraph);
   //    parameters->posGraph = compressedPosGraph;
   // }
   // printf("posGraph updated\n");
   if (sub->numNegInstances > 0) 
   {
   ...
   // printf("new label list computed\n");
   if (compressedNegGraph != NULL)
   ...
   // printf("parameters->labelList updated\n");
...
Graph * CompressFinalGraphs(Substructure *sub, Parameters *parameters,
                         ULONG iteration, BOOLEAN predefinedSub)
{
   ...
   // printf("Compressing final graphs\n");
   ...
   // printf("Best sub level: %lu\n", sub->level);
   // printf("Label %s gets level %lu \n", subLabelString, label.level);
   StoreLabel(& label, labelList);
   if (allowInstanceOverlap &&
       ((InstancesOverlap(sub->instances)) ||
        (InstancesOverlap(sub->negInstances)))) 
   ...
   if (parameters->incremental)
   {
      ...
      for (v = 0; v < increment->numPosVertices; v++)
         if (!g1->vertices[vertexOffset + v].used)
         {
            ...
            g2->vertices[vertexIndex].map = v + vertexOffset;
   ...
   else
   {
      ...
      for (v = 0; v < g1->numVertices; v++)
         if (! g1->vertices[v].used) 
         {
            ...
            g2->vertices[vertexIndex].map = v + vertexOffset;
            g2->vertices[vertexIndex].used = FALSE;
            g2->vertices[vertexIndex].instanceID = VERTEX_UNMAPPED;
            // g2->vertices[vertexIndex].position = gsl_vector_alloc(3);
            // g2->vertices[vertexIndex].orientation = gsl_vector_alloc(3);
            // gsl_vector_memcpy(g2->vertices[vertexIndex].position, g1->vertices[v].position);
            // gsl_vector_memcpy(g2->vertices[vertexIndex].orientation, g1->vertices[v].orientation);
            g2->vertices[vertexIndex].position = g1->vertices[v].position;
            g2->vertices[vertexIndex].orientation = g1->vertices[v].orientation;
            // printf("Position: "); printVector(g2->vertices[vertexIndex].position, 3);
            // printf("Orientation: "); printVector(g2->vertices[vertexIndex].orientation, 3);
      ...
      for (e = 0; e < g1->numEdges; e++)
         if (! g1->edges[e].used) 
         {
            ...
            // printf("v1 label: %s", parameters->labelList->labels[g1->vertices[g1->edges[e].vertex1].label].labelValue.stringLabel);
            // printf(" | v2 label: %s\n", parameters->labelList->labels[g1->vertices[g1->edges[e].vertex2].label].labelValue.stringLabel);
...
void calculateMeanPositions(Graph *graph, InstanceList *instance_list, LabelList * label_list)
{
   InstanceListNode *instance_node;
   instance_node = instance_list->head;

   while(instance_node != NULL)
   {
      calculateMeanPosition(graph, instance_node->instance, label_list);
      instance_node = instance_node->next;
   }
}

void calculateMeanPosition(Graph *graph, Instance *instance, LabelList * label_list)
{
   instance->mean_position = gsl_vector_alloc(3);
   instance->mean_orientation = gsl_vector_alloc(3);

   gsl_vector_set_zero(instance->mean_position);
   BOOLEAN comp_found = FALSE;
   BOOLEAN wall_found = FALSE;
   ULONG comp_ind;
   ULONG wall_ind;
   for(ULONG v=0; v<instance->numVertices; ++v)
   {
      gsl_vector_add(instance->mean_position, graph->vertices[instance->vertices[v]].position);
      if(strcmp(label_list->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel, "350") == 0)
      {
         comp_found = TRUE;
         comp_ind = v;
      }
      else if(strcmp(label_list->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel, "250") == 0)
      {
         wall_found = TRUE;
         wall_ind = v;
      }
   }
   gsl_vector_scale(instance->mean_position, 1.0 / (double)instance->numVertices);

   if(comp_found == TRUE)
   {
      // printf("Compartment found for orientation \n");
      gsl_vector_memcpy(instance->mean_orientation, graph->vertices[instance->vertices[comp_ind]].orientation);
   }
   else if(wall_found == TRUE)
   {
      // printf("Wall found for orientation \n");
      gsl_vector_memcpy(instance->mean_orientation, graph->vertices[instance->vertices[wall_ind]].orientation);
   }
   else
   {
      gsl_vector_memcpy(instance->mean_orientation, graph->vertices[instance->vertices[0]].orientation);
   }
   
   // gsl_vector_memcpy(instance->mean_orientation, graph->vertices[instance->vertices[0]].orientation);
   // gsl_vector_set_zero(instance->mean_orientation);
   // for(ULONG v=0; v<instance->numVertices; ++v)
   // {
   //    gsl_vector_add(instance->mean_orientation, graph->vertices[instance->vertices[v]].orientation);
   // }
   // gsl_vector_scale(instance->mean_orientation, 1.0 / (double)instance->numVertices);
}