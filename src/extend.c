...
SubList *ExtendSub(Substructure *sub, Parameters *parameters)
{
   ...
   InstanceList *separated_instances[sub->numInstances];
   for(int i=0; i<sub->numInstances; ++i)
   {
      separated_instances[i] = AllocateInstanceList();
   }
   newInstanceListNode = newInstanceList->head;
   while (newInstanceListNode != NULL)
   {
      InstanceListNode *new_node = AllocateInstanceListNode(newInstanceListNode->instance);
      new_node->next = separated_instances[newInstanceListNode->instance->parent_instance_id]->head;
      separated_instances[newInstanceListNode->instance->parent_instance_id]->head = new_node;
   ...
   while (newInstanceListNode != NULL) 
   {
      ...
      if (newInstance->minMatchCost != 0.0) 
      {
         ...
         // printf("New sub: %d vertex[0] position: ", newSub->definition->numVertices);
         // for(int vi=0; vi<sub->definition->numVertices; ++vi)
         // {
         //    printVector(newSub->definition->vertices[vi].position,3);
         // }
         // printf("\n");
         if (! MemberOfSubList(newSub, extendedSubs, labelList, parameters->use_pose_cost)) 
         {
            AddPosInstancesToSub(newSub, newInstance, separated_instances, 
                                  parameters,newInstanceListIndex, sub->numInstances);
   ...
   // printf("%d new instances\n", newInstanceListIndex);
   // PrintSubList(extendedSubs, parameters);
   FreeInstanceList(negInstanceList);
   FreeInstanceList(newInstanceList);
   // printf("Extended subs\n");
   return extendedSubs;
}
...
void InterconnectInstances(InstanceList *instanceList, Graph *graph)
{
   InstanceList *newInstanceList;
      ...
      for (v = 0; v < instance->numVertices; v++) 
      {
         ...
         for (e = 0; e < vertex->numEdges; e++) 
         {
            ...
            if (! edge->used) 
            {
               ...
               for (ULONG v2 = 0; v2 < instance->numVertices; v2++)
               {
                  if(source_found && target_found)
                     break;
                  if(edge->vertex1 == instance->vertices[v2])
                     source_found = TRUE;
                  if(edge->vertex2 == instance->vertices[v2])
                     target_found = TRUE; 
               }
               if(source_found && target_found)
               {
                  AddEdgeToInstance(vertex->edges[e], edge, instance);
                  edge->used = TRUE;
                  continue;
               }
            }
      ...
      for (v = 0; v < instance->numVertices; v++) 
      {
         ...
         for (e = 0; e < vertex->numEdges; e++) 
         {
            ...
            if (! edge->used) 
            {
               ...
               newInstance->parent_instance_id = parent_instance_id;
...
//    double matchCost;
//    ULONG counter = 0;
//    InstanceListNode *overlapping_instance;
...
//             // PrintInstance(instanceListNode->instance, 0, posGraph, labelList);
//             if (allowInstanceOverlap ||
//                (! InstanceListOverlap(instance, sub->instances, &overlapping_instance, &overlap_index))) 
//             {
//                /* IF THERE IS OVERLAP SELECT THE ONE WITH LOWER MATCH COST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
//                thresholdLimit = threshold *
//                                 (instance->numVertices + instance->numEdges);
//                instanceGraph = InstanceToGraph(instance, posGraph);
//                // printf("1\n");
//                // printVector(posGraph->vertices[instance->vertices[0]].position,3);
//                //
...
//                   PrintGraph(instanceGraph, labelList);
...
//             //          // printf("Is overlapping instance NULL %d \n", (overlapping_instance == NULL));
//             //          // printf("Current node cost: %f | ", instance->minMatchCost);
//             //          // printf("Overlap node cost: %f\n", overlapping_instance->instance->minMatchCost);
//             //          if(instance->minMatchCost < overlapping_instance->instance->minMatchCost)
//             //          {
//             //             // printf("New instance better than previous one\n");
//             //             // printf("Overlap index: %d\n", overlap_index);
//             //             if(overlap_index == 0)
//             //             {
//             //                InstanceListNode * new_instance_node = AllocateInstanceListNode(instance);
//             //                new_instance_node->next = overlapping_instance->next;
//             //                sub->instances->head = new_instance_node;
//             //                // FreeInstanceListNode(overlapping_instance);
//             //             }
//             //             else
//             //             {
//             //                int ind = 0;
//             //                InstanceListNode * i_node = sub->instances->head;
//             //                while(i_node != NULL && ind < overlap_index-1)
//             //                {
//             //                   i_node = i_node->next;
//             //                   ++ind;
//             //                }
//             //                // printf("Found the instance list node to be replaced\n");
//             //                InstanceListNode * new_instance_node = AllocateInstanceListNode(instance);
//             //                new_instance_node->next = overlapping_instance->next;
//             //                i_node->next = new_instance_node;
//             //                // FreeInstanceListNode(overlapping_instance);
//             //             }
//             //          }
//             //          // InstanceListInsert(instance, sub->instances, FALSE);
...
//             //    if(threshold != 0.0)
//             //    {
//             //       InstanceListNode *overlappingListNode;
//             //       overlappingListNode = overlapping_instances->head;
//             //       double best_match_cost;
//             //       thresholdLimit = threshold * (instance->numVertices + instance->numEdges);
//             //       instanceGraph = InstanceToGraph(instance, posGraph);
//             //       while(overlappingListNode != NULL)
//             //       {
//             //          if (GraphMatch(sub->definition, instanceGraph, labelList,
//             //                         thresholdLimit, & best_match_cost, NULL))
//             //          {
//             //             Graph *overlapping_graph = InstanceToGraph(overlappingListNode->instance, posGraph);
//             //             double overlapping_match_cost;
//             //             if (GraphMatch(sub->definition, overlapping_graph, labelList,
//             //                            thresholdLimit, & overlapping_match_cost, NULL))
//             //             {
//             //                if (overlapping_match_cost < best_match_cost)
//             //                   best_match_cost = overlapping_match_cost;
//             //             }
//             //          }
//             //       }
//             //    }
...
/***** Grouping ******/
void AddPosInstancesToSub(Substructure *sub, Instance *subInstance,
                           InstanceList **separated_instances, Parameters *parameters,
                           ULONG index, ULONG num_groups)
{
   InstanceListNode *instanceListNode;
   Instance *instance;
   Graph *instanceGraph;
   ...
   ULONG counter = 0;
   InstanceListNode *overlapping_instance;
   InstanceListNode *best_matching_instance;
   ...
   subInstance->used = TRUE;
   InstanceListInsert(subInstance, sub->instances, FALSE);
   sub->instance_match_costs = (double *)realloc(sub->instance_match_costs, sizeof(double) * num_groups);
   // (VertexMap *)realloc(filtered_map, filtered_depth * sizeof(VertexMap));
   *(sub->instance_match_costs) = 0.0;
   sub->numInstances++;

   // printf("----------------------\n");
   // printf("Finding instances for:\n");
   // PrintGraph(sub->definition, labelList);
   // PrintInstance(subInstance, 0, posGraph, labelList);
   thresholdLimit = threshold * (subInstance->numVertices + subInstance->numEdges);
   
   for(ULONG i=0; i<num_groups; ++i)
   {
      // printf("Next group %d\n", i);
      InstanceListNode * best_instance_node = NULL;
      double max_best_cost = 99999999999.0;
      double best_cost = max_best_cost;
      instanceListNode = separated_instances[i]->head;
      ...
      while (instanceListNode != NULL) 
      {
         if (instanceListNode->instance != NULL) 
         {
            ...
            int overlap_index = 0;
            if (allowInstanceOverlap || 
               (! InstanceListOverlap(instance, sub->instances, &overlapping_instance, &overlap_index)))   /// SOME ISSUE IN OVERLAP CHECK
            // if(TRUE)
            {
               /* IF THERE IS OVERLAP SELECT THE ONE WITH LOWER MATCH COST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
               // thresholdLimit = threshold *
               //                  (instance->numVertices + instance->numEdges);
               if(instance->numVertices > parameters->maxVertices)
               {
                  instanceListNode = instanceListNode->next;
                  continue;
               }
               ...
               {
                  // printf("2\n");
                  // printf("Checking with:\n");
                  // PrintInstance(instance, 0, posGraph, labelList);
                  // printf("3\n");
                  // PrintGraph(sub->definition, labelList);
                  // printf("3~\n");
                  if (GraphMatch(sub->definition, instanceGraph, labelList,
                                 thresholdLimit, & matchCost, NULL, parameters->use_pose_cost))
                  {
                     ...
                     // sub->numInstances++;
                     // printf("3.4\n");
                     // printf("Cost: current: %f best: %f\n", matchCost, best_cost);

                     if(matchCost < best_cost)
                     {
                        // printf("Found better cost: %f < %f\n", matchCost, best_cost);
                        best_cost = matchCost;
                        best_instance_node = instanceListNode;
                     }
                  }
                  // printf("Match cost: %f / %f\n", matchCost, thresholdLimit);
                  // printf("4\n");
                  // printf("3\n");
               }
               FreeGraph(instanceGraph);
      ...
      if(best_instance_node != NULL && best_cost < max_best_cost)
      {
         // printf("Best cost: %f \n", best_cost);
         InstanceListInsert(best_instance_node->instance, sub->instances, FALSE);
         // printf("Best node inserted\n");
         *(sub->instance_match_costs+sub->numInstances) = best_cost;
         sub->numInstances++;
      }
      // printf("7\n");
   }
...
//    double matchCost;
//    ULONG counter = 0;
//    InstanceList *overlapping_instances;
...
//             else
//             {
//                if(!RemoveOverlappingInstances(instance, sub->instances, overlapping_instances))
//                {
//                   thresholdLimit = threshold *
//                                  (instance->numVertices + instance->numEdges);
//                   instanceGraph = InstanceToGraph(instance, posGraph);
...
//                {
//                   sub->numInstances = 0;
//                   InstanceListNode *trimmed_list_node; 
//                   trimmed_list_node = sub->instances->head;
//                   while(trimmed_list_node != NULL)
//                   {
//                      sub->numInstances++;
//                   }
...
//                   if (threshold != 0.0)
//                   {
//                      InstanceListNode *overlapping_node; 
//                      overlapping_node = overlapping_instances->head;
//                      double best_match_cost;
...
//                         while(overlapping_node != NULL)
//                         {
//                            double overlapping_match_cost;
//                            Graph *overlapping_instance_graph = InstanceToGraph(overlapping_node->instance, posGraph);
//                            if (GraphMatch(sub->definition, overlapping_instance_graph, labelList,
//                                           thresholdLimit, & overlapping_match_cost, NULL))
//                            {
//                               if(overlapping_match_cost < best_match_cost)
//                               {
//                                  best_instance = overlapping_node->instance;
//                                  best_match_cost = overlapping_match_cost;
//                               }
//                            }
...
//                         {
//                            InstanceListInsert(best_instance, sub->instances, FALSE);
//                            sub->numInstances++;
//                         }
//                         else
//                         {
...
//                   FreeGraph(instanceGraph);
//                }
//                FreeInstanceList(overlapping_instances);
//             }
//             // if (allowInstanceOverlap ||
//                // (! InstanceListOverlap(instance, sub->instances, overlapping_instances))) 
//             // {
//             //    /* IF THERE IS OVERLAP SELECT THE ONE WITH LOWER MATCH COST!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
//             //    thresholdLimit = threshold *
//             //                     (instance->numVertices + instance->numEdges);
   ...
   if (instanceList != NULL) 
   {
      ...
      while (instanceListNode != NULL) 
      {
         if (instanceListNode->instance != NULL) 
         {
            ...
            int overlap_index;
            if (allowInstanceOverlap ||
                (! InstanceListOverlap(instance, sub->negInstances, &overlapping_instances, &overlap_index))) 
            {
               thresholdLimit = threshold *
                                (instance->numVertices + instance->numEdges);
