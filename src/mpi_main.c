...
void SubdueMaster(Parameters *parameters)
{
   ...
   while ((iteration <= iterations) && (! done)) 
   {
      ...
      for (i = 1; i <= numPartitions; i++) 
      {
         ...
         if (childSubs[i] != NULL)
            SubListInsert(childSubs[i], subList, numBestSubs, FALSE, labelList, parameters->use_pose_cost);
      }
...
void SubdueChild(Parameters *parameters)
{
   ...
   while ((iteration <= iterations) && (! done)) 
   {
      ...
      else 
      {
         ...
         SubListInsert(subList->head->sub, evalSubs, 0, FALSE, labelList, parameters->use_pose_cost);
      ...
      for (i = 1; i < numPartitions; i++) 
      {
         ...
         else 
         {
            ...
            SubListInsert(evalSub, evalSubs, 0, FALSE, labelList, parameters->use_pose_cost);
