...
Substructure *CopySub(Substructure *sub)
{
   ...
   newSub->level = sub->level;
   // *newSub->label_str = *sub->label_str;
   strcpy(newSub->label_str, sub->label_str);
   
   newSub->instances = (InstanceList *) malloc(sizeof(InstanceList));
   InstanceListNode *inst_iter = (InstanceListNode *) malloc(sizeof(InstanceListNode));
   inst_iter = sub->instances->head;
   InstanceListNode *copy_inst_iter = AllocateInstanceListNode(CopyInstance(sub->instances->head->instance));
   newSub->instances->head = copy_inst_iter;
   inst_iter = inst_iter->next;
   while(inst_iter != NULL)
   {
      InstanceListNode *inst = AllocateInstanceListNode(CopyInstance(inst_iter->instance));
      copy_inst_iter->next = inst;
      copy_inst_iter = inst;
      inst_iter = inst_iter->next;
   }
...
struct avl_table *GetSubTree(Substructure *sub, Parameters *parameters)
{
   ...
   while ((avlTableNode != NULL) && !found)
   {
      if (GraphMatch(sub->definition, avlTableNode->sub->definition,
                     parameters->labelList, 0.0, NULL, NULL, parameters->use_pose_cost))
         found = TRUE;
      else 
