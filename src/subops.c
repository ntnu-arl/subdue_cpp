...
void SubListInsert(Substructure *sub, SubList *subList, ULONG max,
                   BOOLEAN valueBased, LabelList *labelList, BOOLEAN use_pose_cost)
{
   ...
   while ((subIndex != NULL) && (subIndex->sub->value >= sub->value))
   {
      if (subIndex->sub->value == sub->value)
      {
         if (GraphMatch(subIndex->sub->definition, sub->definition,
             labelList, 0.0, NULL, NULL, use_pose_cost))
         {
...
BOOLEAN MemberOfSubList(Substructure *sub, SubList *subList,
                        LabelList *labelList, BOOLEAN use_pose_cost)
{
   SubListNode *subListNode;
   BOOLEAN found = FALSE;
...
Substructure *AllocateSub()
{
   ...
   sub->instance_match_costs = (double *)malloc(sizeof(double));
...
void PrintSub(Substructure *sub, Parameters *parameters)
{
   ...
   if (sub != NULL) 
   {
      printf("Substructure: level = %lu value = %.*g", sub->level, NUMERIC_OUTPUT_PRECISION,
             sub->value);
      ...
      if (outputLevel > 2) 
      {
         ...
         {
            printf("%f ", sub->instance_match_costs[i]);
         }
         printf("\n");
      } 
      else 
   ...
   if (sub->instances != NULL) 
   {
      ...
      while (instanceListNode != NULL) 
      {
         ...
         if (numPosEgs > 1) 
         {
            ...
            printf(": cost: %g\n", instanceListNode->instance->minMatchCost);
         PrintInstance(instanceListNode->instance, vertexOffset, posGraph, labelList);
         printf("Mean position: "); printVector(instanceListNode->instance->mean_position, 3);printf("\n");
         instanceListNode = instanceListNode->next;
         i++;
      }
   }
...
BOOLEAN InstanceListOverlap(Instance *instance, InstanceList *instanceList, InstanceListNode ** overlapping_instance_node, int *overlap_index)
{
   InstanceListNode *instanceListNode;
   // InstanceListNode *overlappingInstanceListNode;
   BOOLEAN overlap = FALSE;
   // printf("Checking instance overlap\n");

   // overlapping_instances = AllocateInstanceList();
   (*overlap_index) = -1;
   if (instanceList != NULL) 
   {
      instanceListNode = instanceList->head;
      ...
      {
         ++(*overlap_index);
         // printf("overlap_index %d \n", *overlap_index);
         if (instanceListNode->instance != NULL)
         {
            
            if (InstanceOverlap(instance, instanceListNode->instance))
            {
               // overlapping_instance_node = AllocateInstanceListNode(instanceListNode->instance);
               // overlapping_instance_node->next = instanceListNode->next;
               // overlapping_instance_node = instanceListNode;
               // overlappingInstanceListNode->next = overlapping_instances->head;
               // overlapping_instances->head = overlappingInstanceListNode;
               // printf("Overlap detected at index %d\n", *overlap_index);
               overlap = TRUE;
            }
         }
         if(!overlap)
...
BOOLEAN RemoveOverlappingInstances(Instance *instance, InstanceList *instanceList, InstanceList * overlapping_instances)
{
   InstanceListNode *instanceListNode;
   InstanceList *trimmed_instance_list;
   BOOLEAN overlap = FALSE;

   overlapping_instances = AllocateInstanceList();
   trimmed_instance_list = AllocateInstanceList();
   if (instanceList != NULL) 
   {
      instanceListNode = instanceList->head;
      while ((instanceListNode != NULL) && (! overlap)) 
      {
         if (instanceListNode->instance != NULL)
         {
            if (InstanceOverlap(instance, instanceListNode->instance))
            {
               InstanceListInsert(instanceListNode->instance, overlapping_instances, FALSE);
               overlap = TRUE;
            }
            else
            {
               InstanceListInsert(instanceListNode->instance, trimmed_instance_list, FALSE);
            }
         }
         instanceListNode = instanceListNode->next;
      }
...
Graph *InstanceToGraph(Instance *instance, Graph *graph)
{
   ...
   for (i = 0; i < instance->numVertices; i++) 
   {
      ...
      newGraph->vertices[i].position = vertex->position;
      newGraph->vertices[i].orientation = vertex->orientation;
      newGraph->vertices[i].degree = vertex->degree;
   }
                     ...
                     double threshold, double *cost)
{
   ...
   if (GraphMatch(g1, g2, labelList, threshold, NULL, subMapping, parameters->use_pose_cost))
   {
...
void markPatternInstances(Substructure * sub, Graph * graph, BOOLEAN val)
{
   InstanceListNode *instance_list_node;
   instance_list_node = sub->instances->head;

   while(instance_list_node != NULL)
   {
      markPatternInstance(instance_list_node->instance, graph, val);
      instance_list_node = instance_list_node->next;
   }
}

void markPatternInstance(Instance *instance, Graph * graph, BOOLEAN val)
{
   for(ULONG v=0; v<instance->numVertices; ++v)
   {
      graph->vertices[instance->vertices[v]].in_pattern = val;
   }
}