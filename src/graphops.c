...
Graph *CopyGraph(Graph *g)
{
    ...
    for (v = 0; v < nv; v++)
    {
        ...
        // gCopy->vertices[v].position = gsl_vector_alloc(3);
        // gCopy->vertices[v].orientation = gsl_vector_alloc(3);
        // gsl_vector_memcpy(gCopy->vertices[v].position, g->vertices[v].position);
        // gsl_vector_memcpy(gCopy->vertices[v].orientation, g->vertices[v].orientation);
        gCopy->vertices[v].position = g->vertices[v].position;
        gCopy->vertices[v].orientation = g->vertices[v].orientation;
        if (numEdges > 0)
        {
...
{
    printf("v %lu ", vertexIndex - vertexOffset);
    PrintLabel(graph->vertices[vertexIndex].label, labelList);
    printf(" "); printVector(graph->vertices[vertexIndex].position, 3);
    // printf("  | imp: %d", labelList->labels[graph->vertices[vertexIndex].label].imp);
    // printf(" "); printVector(graph->vertices[vertexIndex].orientation, 3);
    printf("\n");
}
