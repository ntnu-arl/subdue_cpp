...
ULONG StoreLabel(Label *label, LabelList *labelList)
{
   ...
   { // i.e., label not found
      ...
      // if(label->imp > 0)
      //    labelList->labels[labelList->numLabels].imp = TRUE;
      // else
      //    labelList->labels[labelList->numLabels].imp = FALSE;
      ...
      labelList->labels[labelList->numLabels].level = label->level;
      labelList->numLabels++;
   }
   return labelIndex;
}
...
void calculateSubLevel(Substructure *sub, LabelList *label_list)
{
   ULONG v;

   ULONG sub_level = 1;
   // printf("Sub level before: %lu", sub_level);
   for(v=0; v<sub->definition->numVertices; ++v)
   {
      if(label_list->labels[sub->definition->vertices[v].label].level >= sub_level)
         sub_level = label_list->labels[sub->definition->vertices[v].label].level + 1;
   }
   sub->level = sub_level;
   // printf(" | Sub level after: %lu", sub_level);
}
void calculateSubListLevel(SubList *sub_list, LabelList *label_list)
{
   SubListNode * sub_list_node;
   
   sub_list_node = sub_list->head;
   while(sub_list_node != NULL)
   {
      calculateSubLevel(sub_list_node->sub, label_list);
